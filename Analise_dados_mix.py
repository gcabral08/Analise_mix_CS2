...
# Adicionadas novas funções e melhorias nas existentes.
...
class StatsAnalyzer:
    ...
    def get_h2h_overall(self, player1, player2):
        ...
        p1_wins = h2h_matches_p1['won'].sum()
        total_games = len(h2h_matches_p1)
        ...
        return {
            "Partidas H2H": total_games, f"Vitórias {player1}": int(p1_wins), f"Vitórias {player2}": int(p2_wins),
            f"K/D Total {player1}": f"{p1_total_k}/{p1_total_d}", f"Taxa K/D {player1}": f"{p1_kd:.2f}",
            f"K/D Total {player2}": f"{p2_total_k}/{p2_total_d}", f"Taxa K/D {player2}": f"{p2_kd:.2f}"
        }
    ...
    def get_h2h_by_map(self, player1, player2):
        ...
        for map_name, group in h2h_matches_p1.groupby('map'):
            ...
            results.append({
                'Mapa': map_name, 'Partidas': len(group), f'Vitórias {player1}': int(p1_wins), f'Vitórias {player2}': len(group) - int(p1_wins),
                f'K/D {player1}': f"{p1_stats.K}/{p1_stats.D}", f'K/D {player2}': f"{p2_stats.K}/{p2_stats.D}",
