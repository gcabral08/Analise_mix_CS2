...
class StatsAnalyzer:
    ...
    def get_h2h_by_map(self, player1, player2, start_date, end_date):
        ...
        for map_name, group in h2h_df.groupby('map'):
            ...
            results.append({
                'Mapa': map_name, 'Partidas': total_matches,
                f'Vitórias {player1}': int(p1_wins), f'Vitórias {player2}': total_matches - int(p1_wins),
                f'K/D {player1}': p1_kd, f'K/D {player2}': p2_kd
            })
    ...
    def get_core_player_stats(self, core_players, start_date, end_date):
        ...
        valid_match_ids = [mid for mid, roster in relevant_rosters.items() if core_set.issubset(roster)]
        
        if not valid_match_ids:
            return {"Partidas Juntos": 0, "% de Vitória": 0}, []

        match_info = df[df['match_id'].isin(valid_match_ids)].groupby('match_id').first()
        total_games = len(match_info)
        wins = int(match_info['won'].sum())
        stats = {"Partidas Juntos": total_games, "% de Vitória": (wins / total_games * 100) if total_games > 0 else 0}
        ...
        return stats, sorted(list(possible_next_players - core_set))
...
if uploaded_file:
    try:
        ...
        st.sidebar.markdown("---")
        analysis_type = st.sidebar.radio("Tipo de Análise:", ("Estatísticas Gerais", "Análise de Jogador", "Confronto 1x1 (Contra)", "Montar Time (Cascata)"))
        ...
        if analysis_type == "Estatísticas Gerais":
            ...
            if player_name:
                ...
                if summary:
                    ...
                    trend_chart = create_player_trend_chart(trend_df)
                    if trend_chart: st.image(trend_chart)
                    else: st.info("Não há dados suficientes para gerar o gráfico de tendência.")
            ...
            if player1 and player2:
                ...
                if h2h_overall:
                    ...
                    if not h2h_map_stats.empty:
                        ...
                        st.info("Nenhum confronto H2H encontrado nos mapas do período.")
                else:
                    st.warning("Estes jogadores nunca se enfrentaram no período selecionado.")

        elif analysis_type == "Montar Time (Cascata)":
            ...
            options = player_list
            for i in range(5):
                # Desabilita o selectbox se o anterior não foi escolhido
                is_disabled = (i > 0 and not st.session_state.team_players[i-1])
                
                # Atualiza a lista de opções com base na seleção anterior
                ...
                # Garante que a opção atualmente selecionada esteja na lista de opções
                ...
                st.session_state.team_players[i] = st.sidebar.selectbox(
                    f"Jogador {i+1}", [""] + options, 
            ...
            if len(final_team) >= 2:
                ...
                core_stats, _ = analyzer.get_core_player_stats(final_team, start_date, end_date)
                if core_stats["Partidas Juntos"] > 0:
                    c1, c2 = st.columns(2)
                    c1.metric("Partidas com este núcleo", core_stats["Partidas Juntos"])
                    c2.metric("% de Vitória", f"{core_stats['% de Vitória']:.2f}%")
                else:
                    st.warning("Este núcleo de jogadores nunca jogou junto.")
            else:
                st.info("Selecione pelo menos 2 jogadores para ver as estatísticas do time.")
